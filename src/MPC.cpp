...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the use of actuators (let turn and accerlate smoothly).
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  size_t n_vars = N * n_state + (N-1) * n_actuator;
  ...
  out_vars.push_back(solution.x[delta_start]);
  out_vars.push_back(solution.x[a_start]);
